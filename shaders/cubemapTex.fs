...
vec3 calculateDirectionalLight(vec3 texColor, vec3 specColor){
    // ambient 
...
vec3 calculatePointLight(vec3 texColor, vec3 specColor){ 
  	vec3 ambient = texColor * light.ambient;
...
vec3 calculateSpotLight(vec3 texColor, vec3 specColor){
...
void main() {
  ...
  if (light.lightType == 0)
	  result = calculateDirectionalLight(diffuseColor, specColor);
  if (light.lightType == 1)
	  result = calculatePointLight(diffuseColor, specColor);
  if (light.lightType == 2)
	  result = calculateSpotLight(diffuseColor, specColor);
